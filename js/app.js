...
const getSpace = (headerTableKey, creditoKey) => '  '.repeat((headerTableKey.length - creditoKey.toString().length) < 0
...
const generarCalendario = (headerTable, calendario) => {
  ...
  const calendarioTemplate = calendario.map((rowCuota) => {
    const {
      nroCuota, capital, interes, cuota, saldo,
    } = rowCuota;
    const espacioNroCuota = nroCuota.toString().length === 1
      ? new Array(headerTable.nroCuota.length - 1).join('  ')
      : new Array(headerTable.nroCuota.length - 2).join('  ');
    const espacioCapital = getSpace(headerTable.capital, capital);
    const espacioInteres = getSpace(headerTable.interes, interes);
    const espacioCuota = getSpace(headerTable.cuota, cuota);
    const espacioSaldo = getSpace(headerTable.saldo, saldo);
...
do {
  ...
  const credito = new Credito(PRESTAMO, PLAZO, TEA, SEGURO_DESGRAVAMEN);
